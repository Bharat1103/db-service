...
//
//class JDatabaseServiceImpl(jdbcDAO: DatabaseServiceImpl)(implicit ec: ExecutionContext) {
//  def executeQuery[T](query: String, mapper: java.util.function.Function[DBRow, T]): CompletableFuture[util.List[T]] =
//    jdbcDAO
//      .sele(query)(GetResult(pr => mapper(new DBRow(pr))))
//      .map(_.asJava)
//      .toJava
//      .toCompletableFuture
//
